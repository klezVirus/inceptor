...
class SysWhispers(object):
    ...
    def __init__(self):
        self.typedefs: list = json.load(
            open(os.path.join(get_project_root(), 'syscalls', 'syswhispers', 'data', 'typedefs.json')))
        self.prototypes: dict = json.load(
            open(os.path.join(get_project_root(), 'syscalls','syswhispers', 'data', 'prototypes.json')))
        self.syscall_numbers: dict = json.load(
            open(os.path.join(get_project_root(), 'syscalls','syswhispers', 'data', 'syscall_numbers.json')))
    ...
    def generate(self, function_names: list = (), versions: list = (), basename: str = 'syscalls', debug=False):
    ...
    def _get_typedefs(self, function_names: list) -> list:
        ...
        used_typedefs = []
        for function_name in function_names:
            if function_name not in self.prototypes.keys():
                continue
